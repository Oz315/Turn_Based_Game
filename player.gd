...
func _on_request_action(action: TurnAction):
	...
	current_hint = action.hint(self, action_ctx)
	
	print(current_hint)
	
	level_root.show_hint(current_hint, Vector2i(0, 0))
	is_attacking = true
...
func _input(event):
	...
	if is_attacking:
		var action_ctx = level_root.get_action_context()
		var selected_pos = level_root.tile_map.local_to_map(get_global_mouse_position())
		if current_hint.has(selected_pos):
			current_action.execute(self, selected_pos, action_ctx)
		is_attacking = false
		level_root.move_layer.clear()
		current_hint.clear()
